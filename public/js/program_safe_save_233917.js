...
crosetModule.value("GeneralComponents", {
  ...
  function() {
    return {
      ...
      link: function(scope, element, attrs) {
        var handle, resizing, wrapper;
        resizing = false;
        handle = element.find(".croset-mat-resizer");
        wrapper = element.children(".mat-wrapper");
        scope.startResizing = function() {
          return resizing = true;
        };
        scope.moving = function() {
          var width;
          if (resizing) {
            width = handle.offset().left + handle.width() - element.offset().left;
            element.width(width);
            return console.log("haaaaaaaaaaa", element.parent());
          }
        };
        return scope.stopResizing = function() {
