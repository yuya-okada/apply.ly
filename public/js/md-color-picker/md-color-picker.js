...
angular.module('mdColorPicker', [])
	...
	.service("showColorPicker", ['$timeout', '$mdDialog', 'mdColorPickerHistory', function($timeout, $mdDialog, colorHistory) {
			...
			var that = this;	
			this.showColorPicker = function showColorPicker(callback, defaultColor) {
				that.value = defaultColor
				if ( didJustClose ) {
					return;
				}
				$mdDialog.show({
	...
	//		templateUrl: "mdColorPicker.tpl.html",
			...
			controller: ['$scope', '$element', '$mdDialog', function( $scope, $element, $mdDialog ) {
				...
				$element.bind("click", function(){
					$scope.$apply($scope.showColorPicker)
				})
				$scope.showColorPicker = function showColorPicker() {
					if ( didJustClose ) {
						return;
					}
					$mdDialog.show({
