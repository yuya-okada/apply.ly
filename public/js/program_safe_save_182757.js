...
crosetModule.value("GeneralComponents", {
  ...
  function() {
    return {
      ...
      link: function(scope, element, attrs) {
        var mousePosition, resizing, wrapper;
        resizing = false;
        mousePosition = {};
        ...
        document.onmousemove = function($event) {
          if (resizing) {
            ...
            scope.width = wrapper.width() + $event.pageX - mousePosition.x;
            scope.height = wrapper.height() + $event.pageY - mousePosition.y;
            wrapper.width(scope.width);
            wrapper.height(scope.height);
            mousePosition = {
              x: $event.pageX,
              y: $event.pageY
            };
